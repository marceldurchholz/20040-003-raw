define(['underscore', 'Backbone', 'text!parser/simplevideo/simplevideo.html'],
    function (_, Backbone, simplevideoTemplate) {

        var simplevideoViewVar = Backbone.View.extend({
			...
			initialize:function(options) {
				// console.log('initializing simplevideoView.js  w/  window.ajaxLoader: '+window.ajaxLoader);
				_this = this;
				...
				_this.options.query_vars.videodetailsid = _this.options.query_vars.videodetailsid || "";
			},
			parseData:function() {
				var d = $.Deferred();
				var _this = this;
					...
					$.when(window.MobileApp.myrouter.getViewByRouteForPush(_this.options.sidemenuModel.attributes.urloffline+'View',_this.options)).done(
						function(viewToPush) {
							...
							try {
								...
								console.log('Error during simplevideo module parsing:');
								console.log(e);
								_this.$el.html(_.template(simplevideoTemplate,{page_vars:_this.options}).replace('	','').replace(/\n|\r|\t/g, ""));
								$(_this.$el).trigger('create');
								d.resolve(_this);
							} finally {	
							}
