...
(function (undefined) {
  ...
  var root = "http://s15944029.onlinehome-server.info:5000";
...
dpd.appoptions.once = function(ev, fn) {
  return dpd.once("appoptions" + ":" + ev, fn);
}
dpd.appoptions.off = function(ev, fn) {
  return dpd.off("appoptions" + ":" + ev, fn);
}
dpd.appoptions.rename = function (path, body, fn) {
  dpd.appoptions.exec("rename", path, body, fn);
}
...
dpd.usergroups.once = function(ev, fn) {
  return dpd.once("usergroups" + ":" + ev, fn);
}
dpd.usergroups.off = function(ev, fn) {
  return dpd.off("usergroups" + ":" + ev, fn);
}
dpd.usergroups.rename = function (path, body, fn) {
  dpd.usergroups.exec("rename", path, body, fn);
}
