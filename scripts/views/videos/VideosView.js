define(['jquery', 'underscore', 'Backbone', 'text!views/template/VideosView.html'],
    function ($, _, Backbone, videosTemplate) {
        var VideosViewVar = Backbone.View.extend({
			...
			initialize:function(options) {
				console.log('initialize VideosView.js');
				// alert('initialize VideosView.js');
				_this = this;
				...
				_this.options.streamData.videosArray = _this.options.streamData.videosArray || new Array();
			...
			fetchData:function(options) {
				...
				console.log('fetch: VideosView.js');
				if (_this.options.query_vars.userid!=undefined && _this.options.query_vars.userid!="") {
					...
					$.when(collectUserData(_this.options.query_vars.userid) , collectVideosArray(_this.options.query_vars.userid)).done(
						function(userArray,videosArray) {
							_this.options.streamData.userArray = new Array(userArray);
							_this.options.streamData.videosArray = videosArray;
							d.resolve(_this);
						}
					);
				}
					...
					function(_this) {
						...
						require([_this.options.sidemenuModel.get('templateUrl')], function(_foundTemplateContent) {
							try {
								...
								require(['text!views/template/VideosView.html'], function(_foundTemplateContent) {
									var _finalContent = _.template(_foundTemplateContent,{page_vars:_this.options}).replace('	','').replace(/\n|\r|\t/g, "");
									_this.$el.html(_finalContent);
									$(_this.$el).trigger('create');
									d.resolve(_this);
