define(['jquery', 'underscore', 'Backbone', 'text!views/template/UsersView.html'],
    function ($, _, Backbone, usersTemplate) {
        var UsersViewVar = Backbone.View.extend({
			...
			initialize:function(options) {
				...
				_this.options.streamData.usersArray = _this.options.streamData.usersArray || new Array();
			...
			fetchData:function(options) {
				...
				if (_this.options.query_vars.userid!=undefined && _this.options.query_vars.userid!="") {
					$.when(collectUserData(_this.options.query_vars.userid),collectUsersArray(_this.options.query_vars.userid)).done(
						function(userArray,usersArray) {
							_this.options.streamData.userArray = new Array(userArray);
							_this.options.streamData.usersArray = usersArray;
							d.resolve(_this);
						}
					);
				}
					...
					function(_this) {
						...
						require([_this.options.sidemenuModel.get('templateUrl')], function(_foundTemplateContent) {
							try {
								...
								require(['text!views/template/UsersView.html'], function(_foundTemplateContent) {
									var _finalContent = _.template(_foundTemplateContent,{page_vars:_this.options}).replace('	','').replace(/\n|\r|\t/g, "");
									_this.$el.html(_finalContent);
									$(_this.$el).trigger('create');
									d.resolve(_this);
