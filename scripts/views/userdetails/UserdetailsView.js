define(['jquery', 'underscore', 'Backbone', 'text!views/template/UserdetailsView.html'],
    function ($, _, Backbone, userdetailsTemplate) {
        var UserdetailsViewVar = Backbone.View.extend({
			...
			initialize:function(options) {
				...
				_this.options.streamData.userdetailsData = _this.options.streamData.userdetailsData || new Array();
					...
					function(_this) {
						...
						require([_this.options.sidemenuModel.get('templateUrl')], function(_foundTemplateContent) {
							try {
								...
								require(['text!views/template/UserdetailsView.html'], function(_foundTemplateContent) {
									var _finalContent = _.template(_foundTemplateContent,{page_vars:_this.options}).replace('	','').replace(/\n|\r|\t/g, "");
									_this.$el.html(_finalContent);
									$(_this.$el).trigger('create');
									d.resolve(_this);
